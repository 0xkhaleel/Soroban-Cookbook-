...
Handling multiple networks correctly is crucial for a smooth developer and user experience. Each Stellar network (Mainnet, Testnet, Futurenet) has a unique passphrase and RPC endpoint.
...
Apps should allow users to connect to different RPC providers. Standard SDF endpoints include:
...
When building transactions, you must use the correct passphrase:

```ts
import { Networks } from "@stellar/stellar-sdk";

const networkPassphrase = Networks.TESTNET; // Or custom string for Futurenet
```
...
The following example demonstrates a complete flow: connecting the wallet, building a transaction, signing it, and submitting it to the network.
...
async function runExampleWorkflow(contractId: string, method: string, args: any[]) {
  try {
    // 1. Connect Wallet
    const allowed = await requestAccess();
    if (!allowed) throw new Error("Wallet access denied");
    const userAddress = await getAddress();

    // 2. Initialize Server and Network
    const server = new Server("https://soroban-testnet.stellar.org");
    ...
    // 4. Build Contract Invocation
    const contract = new Contract(contractId);
    let tx = new TransactionBuilder(account, {
      fee: "100", // Placeholder fee, will be updated during preparation
      networkPassphrase,
    })
    ...
    // 5. Prepare Transaction (Simulation)
    // This step is critical for Soroban to calculate resource usage (footprint, instructions)

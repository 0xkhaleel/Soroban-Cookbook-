...
pub struct UserProfile {
    ...
    /// User's display name
    pub name: String,
    /// User's email (empty string when absent)
    pub email: String,
    /// User's avatar hash (empty string when absent)
...
pub struct Transaction {
    ...
    pub timestamp: u64,
    /// Transaction memo (empty string when absent)
    pub memo: String,
    /// Transaction status
...
pub struct Portfolio {
    ...
    /// Portfolio name
    pub name: String,
    /// Portfolio description (empty string when absent)
...
impl CustomStructsContract {
    ...
    pub fn create_user_profile(
        ...
        let profile = UserProfile {
            ...
            name,
            email: email.unwrap_or_else(|| String::from_str(&env, "")),
    ...
    pub fn update_user_profile(
        ...
        if let Some(new_avatar) = avatar_hash {
            profile.avatar_hash = new_avatar;
        }
    ...
    pub fn create_portfolio(
        ...
        let portfolio = Portfolio {
            owner: owner.clone(),
            name: name.clone(),
            description: description.unwrap_or_else(|| String::from_str(&env, "")),
    ...
    pub fn validate_struct(_env: Env, profile: UserProfile) -> Result<bool, ContractError> {
        ...
        if !profile.email.is_empty() && profile.email.len() > 255 {
            return Err(ContractError::InvalidFieldValue);
        }

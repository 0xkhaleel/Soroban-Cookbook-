...
fn setup(env: &Env) -> CustomStructsContractClient<'_> {
...
fn test_basic_struct_creation() {
    ...
    let profile = UserProfile {
        ...
        name: name.clone(),
        email: email.clone(),
        avatar_hash: String::from_str(&env, ""),
        reputation: 100,
        verified: false,
...
fn test_portfolio_struct() {
    ...
    let portfolio = Portfolio {
        ...
        description: String::from_str(&env, "A diversified portfolio"),
        holdings: vec![
            &env,
            AssetHolding {
                asset: AssetInfo {
...
fn test_user_profile_management() {
    ...
    let profile = client.create_user_profile(&user, &name, &Some(email.clone()));
    assert_eq!(profile.address, user);
    assert_eq!(profile.name, name);
    assert_eq!(profile.email, email.clone());
    assert_eq!(profile.reputation, 0);
    assert!(!profile.verified);

    let retrieved = client.get_user_profile(&user);
    assert_eq!(profile, retrieved);

    let new_name = String::from_str(&env, "Charlie Updated");
    let updated = client.update_user_profile(&user, &Some(new_name.clone()), &None, &None);
...
fn test_portfolio_management() {
    ...
    let portfolio = client.create_portfolio(
        &owner,
        &portfolio_name,
    ...
    let retrieved = client.get_portfolio(&owner, &portfolio_name);
    ...
    client.add_asset_to_portfolio(&owner, &portfolio_name, &asset, &100_000_000, &50_000);

    let updated = client.get_portfolio(&owner, &portfolio_name);
...
fn test_extended_profile() {
    ...
    let extended = client.create_extended_profile(&user, &name, &language);
    ...
    let retrieved = client.get_extended_profile(&user);
...
fn test_struct_validation() {
    ...
    let valid = UserProfile {
        ...
        email: String::from_str(&env, "valid@example.com"),
        avatar_hash: String::from_str(&env, ""),
        reputation: 500,
        verified: false,
...
fn test_serialization() {
    ...
    assert_eq!(client.serialize_struct(&profile), 12345);
}
...
fn test_portfolio_value_calculation() {
    ...
    client.create_portfolio(&owner, &portfolio_name, &None, &PortfolioType::Balanced);
    ...
    let value = client.calculate_portfolio_value(&owner, &portfolio_name);

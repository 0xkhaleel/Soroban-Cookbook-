...
#[contract]
pub struct FacetAdderContract;

#[contractimpl]
impl FacetAdderContract {
    /// Initialize the Adder Facet with the Diamond Proxy address
    pub fn init_adder(env: Env, diamond: Address) -> Result<(), SecurityError> {
    ...
    /// Sample Add method. Can only be invoked via Diamond Proxy.
    pub fn add(env: Env, caller: Address, a: i128, b: i128) -> Result<i128, SecurityError> {
        caller.require_auth();
